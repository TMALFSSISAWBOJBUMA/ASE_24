...
static int8_t writeRegisters(TMAG5273_device_handle_t sensor, uint8_t regAddress, uint8_t *dataBuffer, uint8_t numBytes);
...
/// @param bus I2C bus handle to use
/// @param sensor TMAG5273 device handle to store the device information
...
{
    sensor->bus = bus;                     // Chooses the wire port of the device
    sensor->deviceAddress = sensorAddress; // Sets the address of the device
    ...
    TMAG5273_setOperatingMode(sensor, TMAG5273_CONTINUOUS_MEASURE_MODE);
    TMAG5273_setXYAxisRange(sensor, TMAG5273_RANGE_80MT);
    TMAG5273_setZAxisRange(sensor, TMAG5273_RANGE_80MT);

    if (TMAG5273_getError(sensor) != 0)
    {
        return ESP_FAIL;
    }

    if (TMAG5273_getLowPower(sensor) != TMAG5273_LOW_ACTIVE_CURRENT_MODE)
    {
        return ESP_FAIL;
    }

    if (TMAG5273_getOperatingMode(sensor) != TMAG5273_CONTINUOUS_MEASURE_MODE)
    {
        return ESP_FAIL;
    }

    if (TMAG5273_getMagneticChannel(sensor) != TMAG5273_X_Y_Z_ENABLE)
    {
        return ESP_FAIL;
    }

    if (TMAG5273_getTemperatureEN(sensor) != TMAG5273_TEMPERATURE_ENABLE)
    {
        return ESP_FAIL;
    }

    if (TMAG5273_getAngleEn(sensor) != TMAG5273_NO_ANGLE_CALCULATION)
    {
        return ESP_FAIL;
    }
...
int8_t TMAG5273_isConnected(TMAG5273_device_handle_t sensor)
{
    ...
    if (TMAG5273_getManufacturerID(sensor) != TMAG5273_DEVICE_ID_VALUE)
    {
        return -1;
    }
...
static int8_t writeRegisters(TMAG5273_device_handle_t sensor, uint8_t regAddress, uint8_t *dataBuffer, uint8_t numBytes)
{
    uint8_t data[1 + numBytes];
    data[0] = regAddress;
    for (uint8_t i = 0; i < numBytes; i++)
    {
        data[i + 1] = dataBuffer[i];
    }
    return i2c_master_transmit(sensor->self, data, numBytes + 1, 100) == ESP_OK ? 0 : -1;
}
...
{
    uint8_t regVal = 0;
    readRegisters(sensor, regAddress, &regVal, 2);
    return regVal;
}
...
static uint8_t writeRegister(TMAG5273_device_handle_t sensor, uint8_t regAddress, uint8_t data)
{
    ...
    writeRegisters(sensor, regAddress, &data, 1);
    return data;
}
...
{
    writeRegister(sensor, TMAG5273_REG_INT_CONFIG_1, 0X64);
    writeRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2, 0X23);

    return TMAG5273_getError(sensor);
}
...
int8_t TMAG5273_readWakeUpAndSleepData(TMAG5273_device_handle_t sensor, float *xVal, float *yVal, float *zVal, float *temperature)
{
    ...
    // Read 4 bits of data
    readRegisters(sensor, TMAG5273_REG_T_MSB_RESULT, wakeupRegisterRead, 4);
...
{
    uint8_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1);
    ...
    if (crcMode == 0)
    {
        ...
        writeRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1, mode);
    }
...
{
    uint16_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
    ...
    if (threshold == 0) // 0b00
    {
        ...
        // Writes a 0 to bit 5
        bitWrite(mode, 5, 0);
        writeRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2, mode);
    }
...
{
    uint16_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_1);
    ...
    if (channelMode == 0X0) // 0b0000
    {
        ...
        writeRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_1, mode);
...
{
    uint16_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
    ...
    if (threshDir == 0X0) // 0b0
    {
        ...
        writeRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2, mode);
    }
...
int8_t TMAG5273_setMagneticGain(TMAG5273_device_handle_t sensor, float magneticGain)
{
    ...
    writeRegister(sensor, TMAG5273_REG_MAG_GAIN_CONFIG, magneticGainReg);
...
int8_t TMAG5273_setMagneticOffset1(TMAG5273_device_handle_t sensor, float offset1)
{
    ...
    writeRegister(sensor, TMAG5273_REG_MAG_OFFSET_CONFIG_1, magOffset);
...
int8_t TMAG5273_setMagneticOffset2(TMAG5273_device_handle_t sensor, float offset2)
{
    ...
    uint8_t rangeValZ = TMAG5273_getZAxisRange(sensor);
    uint8_t rangeZ = 0;
    if (rangeValZ == 0)
    {
        rangeZ = 40;
    ...
    writeRegister(sensor, TMAG5273_REG_MAG_OFFSET_CONFIG_2, magOffset);
...
int8_t TMAG5273_setXThreshold(TMAG5273_device_handle_t sensor, float xThreshold)
{
    ...
    writeRegister(sensor, TMAG5273_REG_X_THR_CONFIG, threshold);
...
int8_t TMAG5273_setYThreshold(TMAG5273_device_handle_t sensor, float yThreshold)
{
    ...
    // Write the correct value to the register
    writeRegister(sensor, TMAG5273_REG_Y_THR_CONFIG, threshold);
...
/// @return Error code (0 is success, negative is failure, positive is warning)
int8_t TMAG5273_setZThreshold(TMAG5273_device_handle_t sensor, float zThreshold)
{
    // See which Z axis range is currently set
    uint8_t range = TMAG5273_getZAxisRange(sensor);
    // Mulyiply raw value by threshold equation in datasheet
    int8_t threshold = zThreshold * 128 / (40 * (1 + range));
    // Write the correct value to the register
    writeRegister(sensor, TMAG5273_REG_Z_THR_CONFIG, threshold);
...
int8_t TMAG5273_setTemperatureThreshold(TMAG5273_device_handle_t sensor, int8_t tempThresh)
{
    ...
    writeRegister(sensor, TMAG5273_REG_T_CONFIG, (tempThresh << 1) | 0x01);
...
{
    uint16_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_T_CONFIG);
    ...
    if (temperatureEnable == 0) // 0b0
    {
        ...
        writeRegister(sensor, TMAG5273_REG_T_CONFIG, mode);
    }
...
{
    uint16_t mode = 0;
    mode = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
    ...
    if (interruptEnable == 0) // 0b0
    {
        ...
        writeRegister(sensor, TMAG5273_REG_INT_CONFIG_1, mode);
    }
...
int8_t TMAG5273_setI2CAddress(TMAG5273_device_handle_t sensor, uint8_t address)
{
    ...
    writeRegister(sensor, TMAG5273_REG_I2C_ADDRESS, (address << 1) | 0x01);
    // Set the device address to the new address chosen
    sensor->deviceAddress = address;
...
{
    uint16_t addReg = 0;
    addReg = readRegister(sensor, TMAG5273_REG_I2C_ADDRESS);
    ...
    if (addressEnable == 0)
    {
        ...
        bitWrite(addReg, 0, 0);
        writeRegister(sensor, TMAG5273_REG_I2C_ADDRESS, addReg);
    }
...
int8_t TMAG5273_setOscillatorError(TMAG5273_device_handle_t sensor, bool oscError)
{
    ...
    deviceStatusReg = readRegister(sensor, TMAG5273_REG_DEVICE_STATUS);
...
{
    uint8_t getCRC = 0;
    getCRC = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1);
...
{
    uint8_t magTemp = 0;
    magTemp = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1);
...
{
    uint8_t convAv = 0;
    convAv = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1);
...
uint8_t TMAG5273_getReadMode(TMAG5273_device_handle_t sensor)
{
    ...
    uint8_t readModeReg = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_1);
...
uint8_t TMAG5273_getIntThreshold(TMAG5273_device_handle_t sensor)
{
    ...
    interruptThreshold = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
...
uint8_t TMAG5273_getLowPower(TMAG5273_device_handle_t sensor)
{
    ...
    lowPowerMode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
...
uint8_t TMAG5273_getGlitchFiler(TMAG5273_device_handle_t sensor)
{
    ...
    glitchMode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
...
uint8_t TMAG5273_getTriggerMode(TMAG5273_device_handle_t sensor)
{
    ...
    triggerMode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
...
{
    uint8_t opMode = 0;
    opMode = readRegister(sensor, TMAG5273_REG_DEVICE_CONFIG_2);
...
uint8_t TMAG5273_getMagneticChannel(TMAG5273_device_handle_t sensor)
{
    ...
    magChannel = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_1);
...
{
    uint8_t sleepReg = 0;
    sleepReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_1);
...
uint8_t TMAG5273_getMagDir(TMAG5273_device_handle_t sensor)
{
    ...
    magDirectionReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
...
uint8_t TMAG5273_getMagnitudeChannelSelect(TMAG5273_device_handle_t sensor)
{
    ...
    magGainReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
...
uint8_t TMAG5273_getMagneticGain(TMAG5273_device_handle_t sensor)
{
    ...
    magneticGainReg = readRegister(sensor, TMAG5273_REG_MAG_GAIN_CONFIG);
...
{
    int8_t magOffset1 = 0;
    magOffset1 = readRegister(sensor, TMAG5273_REG_MAG_OFFSET_CONFIG_1);
...
{
    int8_t magOffset2 = 0;
    magOffset2 = readRegister(sensor, TMAG5273_REG_MAG_OFFSET_CONFIG_2);
...
{
    uint8_t angleReg = 0;
    angleReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
...
uint8_t TMAG5273_getXYAxisRange(TMAG5273_device_handle_t sensor)
{
    ...
    XYrangeReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
...
{
    uint8_t ZrangeReg = 0;
    ZrangeReg = readRegister(sensor, TMAG5273_REG_SENSOR_CONFIG_2);
...
{
    int8_t zThresh = 0;
    zThresh = readRegister(sensor, TMAG5273_REG_Z_THR_CONFIG);
    uint8_t range = TMAG5273_getZAxisRange(sensor);
...
float TMAG5273_getTemperatureThreshold(TMAG5273_device_handle_t sensor)
{
    ...
    tempThreshReg = readRegister(sensor, TMAG5273_REG_T_CONFIG);
...
{
    uint8_t tempENreg = 0;
    tempENreg = readRegister(sensor, TMAG5273_REG_T_CONFIG);
...
uint8_t TMAG5273_getInterruptResult(TMAG5273_device_handle_t sensor)
{
    ...
    intRsltReg = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
...
{
    uint8_t threshReg = 0;
    threshReg = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
...
uint8_t TMAG5273_getIntPinState(TMAG5273_device_handle_t sensor)
{
    ...
    intStateReg = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
...
uint8_t TMAG5273_getInterruptMode(TMAG5273_device_handle_t sensor)
{
    ...
    intModeReg = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
...
uint8_t TMAG5273_getMaskInt(TMAG5273_device_handle_t sensor)
{
    ...
    maskIntReg = readRegister(sensor, TMAG5273_REG_INT_CONFIG_1);
...
{
    uint8_t convReg = 0;
    convReg = readRegister(sensor, TMAG5273_REG_CONV_STATUS);
...
uint8_t TMAG5273_getI2CAddress(TMAG5273_device_handle_t sensor)
{
    ...
    uint8_t address = readRegister(sensor, TMAG5273_REG_I2C_ADDRESS);
...
{
    uint8_t deviceReg = 0;
    deviceReg = readRegister(sensor, TMAG5273_REG_DEVICE_ID);
...
uint16_t TMAG5273_getManufacturerID(TMAG5273_device_handle_t sensor)
{
    ...
    readRegisters(sensor, TMAG5273_REG_MANUFACTURER_ID_LSB, databuffer, 2);
...
uint8_t TMAG5273_getDeviceStatus(TMAG5273_device_handle_t sensor)
{
    ...
    uint8_t deviceStatusReg = readRegister(sensor, TMAG5273_REG_DEVICE_STATUS);
...
int8_t TMAG5273_getError(TMAG5273_device_handle_t sensor)
{
    ...
    uint8_t statusReg = TMAG5273_getDeviceStatus(sensor);
...
float TMAG5273_getTemp(TMAG5273_device_handle_t sensor)
{
    ...
    readRegisters(sensor, TMAG5273_REG_T_MSB_RESULT, databuffer, 2);
...
{
    int8_t xLSB = readRegister(sensor, TMAG5273_REG_X_LSB_RESULT);
    int8_t xMSB = readRegister(sensor, TMAG5273_REG_X_MSB_RESULT);
...
float TMAG5273_getYData(TMAG5273_device_handle_t sensor)
{
    ...
    yLSB = readRegister(sensor, TMAG5273_REG_Y_LSB_RESULT);
    yMSB = readRegister(sensor, TMAG5273_REG_Y_MSB_RESULT);
...
float TMAG5273_getZData(TMAG5273_device_handle_t sensor)
{
    ...
    zLSB = readRegister(sensor, TMAG5273_REG_Z_LSB_RESULT);
    zMSB = readRegister(sensor, TMAG5273_REG_Z_MSB_RESULT);
...
float TMAG5273_getAngleResult(TMAG5273_device_handle_t sensor)
{
    ...
    angleLSB = readRegister(sensor, TMAG5273_REG_ANGLE_RESULT_LSB) & 0b11111111;
    angleMSB = readRegister(sensor, TMAG5273_REG_ANGLE_RESULT_MSB);
...
float TMAG5273_getMagnitudeResult(TMAG5273_device_handle_t sensor)
{
    ...
    magReg = readRegister(sensor, TMAG5273_REG_MAGNITUDE_RESULT);
